...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
    	...
    	if gain > previous_high:
    		print(str(gain) + " point" + ("" if gain == 1 else "s") + "! That's the biggest gain yet for Player " + str(who))
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    k, num, avg = 1, 1, 0
    while k <= 10:
    	temp = make_averaged(roll_dice, num_samples)(k, dice) 
    	if temp > avg:
    		avg, num = temp, k
    	k += 1
    return num
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if free_bacon(opponent_score) >= margin:
    	return 0
    elif is_swap(score + free_bacon(opponent_score), opponent_score) and opponent_score - score >= margin:
    	return 0
    return num_rolls
    # END PROBLEM 10
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if free_bacon(opponent_score) >= margin:
    	return 0
    elif is_swap(score + free_bacon(opponent_score), opponent_score) and opponent_score - score > 0:
    	return 0
    return num_rolls
    # END PROBLEM 11
